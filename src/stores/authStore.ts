...
interface LoginResponse {
  success: boolean
  reason?: 'user_missing' | 'password_error' | 'network_error' | 'captcha_error' | 'account_locked' | 'captcha_required' | 'ip_cooldown'
  message?: string
  attempts?: number
  is_locked?: boolean
}
  ...
  async function login(username: string, password: string, captcha_code?: string, captcha_id?: string): Promise<LoginResponse> {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        } else if (data.error?.includes('冷却') || data.error?.includes('cooldown')) {
          // IP冷却
          return {
            success: false,
            reason: 'ip_cooldown',
            message: data.error || '尝试次数过多，请稍后再试',
            attempts: 5
          }
